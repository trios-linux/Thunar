...
if [ ! -f $PKG ]; 
then
	wget http://archive.xfce.org/src/xfce/$NAMEL/1.6/$PKG
else
...
then
	if [ ! -f $PKG ]; 
	then
		wget wget http://archive.xfce.org/src/xfce/$NAMEL/1.6/$PKG
	else
		echo "$PKG exists, skipping download"
	fi
...
then
	...
	tar xvjf $PKG --strip 1 && mv -f -v $PKG ../$NAMEL\_$VER.orig.tar.bz2 && rm -f ./README.md && rm -f -- "$0"
fi
